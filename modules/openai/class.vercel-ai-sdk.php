...
class Vercel_AI_SDK
{
	...
	{
		$this->sendStreamPart('i:', json_encode(['data' => $data]));
	}

	public function sendReasoningSignature(string $signature): void
	{
		$this->sendStreamPart('j:', json_encode(['signature' => $signature]));
	}
	...
	{
		$payload = ['toolCallId' => $toolCallId, 'toolName' => $toolName, 'args' => $args];
	...
	{
		$payload = ['toolCallId' => $toolCallId, 'result' => $result];
		$this->sendStreamPart('a:', json_encode($payload));
	}

	public function startStep(string $messageId): void
	{
		$this->sendStreamPart('f:', json_encode(['messageId' => $messageId]));
	}
