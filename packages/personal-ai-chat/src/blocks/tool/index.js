...
function EditComponent( { attributes, setAttributes } ) {
	...
	useEffect( () => {
		if (
			hasOutputSchema &&
			outputSchemaFields &&
			( ! attributes.outputFields ||
				attributes.outputFields.length === 0 )
		) {
	...
	const renderOutputFields = () => {
		...
		return (
			<div key="output-fields" style={ { marginTop: '10px' } }>
				<strong>{ __( 'Output Fields', 'personal-ai-chat' ) }</strong>
				<div
					style={ {
						marginTop: '5px',
						display: 'flex',
	...
	const renderOutputFormat = () => {
		...
		return (
			<SelectControl
				key="output-format"
				label={ __( 'Output Format', 'personal-ai-chat' ) }
		...
		<div { ...blockProps } className="wp-block pos-ai-tool">
			<Placeholder
				...
				label={
					attributes.tool
						? attributes.tool
						: __( 'WP Ability', 'personal-ai-chat' )
				}
				instructions={ __(
					'Output the result of a WordPress Ability.',
					'personal-ai-chat'
				) }
			>
				<div
				...
				>
					<SelectControl
						...
						options={ abilities
							...
							.concat( {
								label: __( 'None', 'personal-ai-chat' ),
								value: '',
							} ) }
