...
- Enable the native `wp_knowledge_type` taxonomy UI in Gutenberg so the current note's complete term assignment can be edited, including system and container terms.
...
- Gutenberg exposes the native Knowledge Type panel for editing the current note's `wp_knowledge_type` terms.
