...
const Edit = ( props ) => {
    ...
    const { postTitle, postMeta, highlights, postContent } = useSelect( ( select ) => {
        const blocks = select('core/block-editor').getBlocks();
        return {
            postTitle: select('core/editor').getEditedPostAttribute('title'),
            postMeta: select('core/editor').getEditedPostAttribute('meta'),
            postContent: blocks.filter( block => ( block.name !== 'pos/readwise' && block.name !== 'pos/book-summary' ) ).map( block => getBlockContent( block ) ).join( "\n" ),
            highlights: blocks.filter( block => block.name === 'pos/readwise' ).map( block => block?.attributes?.content ).filter( Boolean ).join( "\n-------\n" ),
        };
    });
            ...
            <InspectorControls>
				<PanelBody title={ 'Book Summarizer Config' }>
                    <TextareaControl
                        label="System Prompt"
                        value={ systemPrompt }
                        onChange={ newContent => setSystemPrompt( newContent ) }
                    />
				</PanelBody>
			</InspectorControls>
		</div>
