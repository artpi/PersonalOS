...
const Edit = ( props ) => {
	...
	const localEmbeddedContent = useSelect(
		( select ) => {
			...
			const childrenBlocks =
				getBlocksByClientId( children?.props?.clientId )[ 0 ]
					?.innerBlocks || [];
	...
	const remoteEmbedded = useSelect(
		( select ) => {
			if ( ! note_id ) {
				return null;
			}
	...
	useEffect( () => {
		if ( ! note_id ) {
			...
			saveEntityRecord( 'postType', 'notes', {
				title: `Note from ${ currentPostTitle } ${ today }`,
				status: 'draft',
	...
	useEffect( () => {
		...
		editEntityRecord( 'postType', 'notes', note_id, {
			content: localEmbeddedContent,
		} );
